...
```bash
npx kodkod@latest express-app --framework express --database postgresql --orm prisma
```

### Scaffold with Hono + Drizzle + PostgreSQL

```bash
npx kodkod@latest hono-app --framework hono --database postgresql --orm drizzle
```
...
```bash
cd my-api
npx kodkod generate route product
```
